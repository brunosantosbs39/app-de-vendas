...
- [x] Alterar o container principal `flex-1 p-8 space-y-10 pb-32` para usar paddings responsivos (ex: `sm:p-8 p-6`) e aumentar o padding inferior (ex: `pb-44`) para não colidir com a `BottomNav`.

### 2. Cards de Compromisso
- [x] **Indicador de Horário:** Alterar de `w-20` para `sm:w-20 w-16` (ou `w-[4.5rem]`) para liberar espaço horizontal.
- [x] **Conteúdo Principal:** Reduzir o `p-8` interno para `sm:p-8 p-5`.
- [x] **Títulos:** Alterar o tamanho da fonte do nome do cliente (`text-2xl`) para `sm:text-2xl text-xl`.
- [x] **Botões de Ação:** Garantir que os botões do rodapé do card caibam em telas menores. Se necessário, ajustar o botão "Concluir" para usar padding menor em mobile.

### 3. Card de Otimização (Empty State / Productivity)
- [x] Reduzir o padding de `p-10` para `sm:p-10 p-6`.
