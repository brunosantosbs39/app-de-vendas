...
- [ ] Alterar `bg-slate-50` para `bg-[#0F0F0F]` no `body`.
- [ ] Ajustar `text-slate-900` para `text-[#F8F8F8]` para manter a compatibilidade com o modo escuro globalmente.
- [ ] Remover ou ajustar o `pb-16` do body se estiver causando espaçamento indesejado com a navegação fixa.

### 2. Navegação (components/layout/BottomNav.tsx)
- [ ] Refinar a lógica de ocultação: Atualmente só oculta em `/login`. Deve-se garantir que não apareça em nenhuma tela de "boas-vindas" ou "auth" caso existam.
- [ ] Ajustar o posicionamento para garantir que não haja "gap" entre o conteúdo e a barra de navegação.
