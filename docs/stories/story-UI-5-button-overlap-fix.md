...
- [ ] Aumentar o `pb-32` no container principal para `pb-44` ou superior, garantindo folga para a `BottomNav` (que tem ~20rem de altura + padding).
- [ ] Opcionalmente, adicionar uma `div` de respiro (`h-24`) após o botão para garantir o empurrão visual.

### 2. Estilo do Botão (app/page.tsx)
- [ ] Adicionar um `z-index` relativo ao container do botão para garantir que ele não sofra interferências visuais de outros elementos de fundo, embora a `BottomNav` deva continuar com o maior `z-index` (`z-100`).
