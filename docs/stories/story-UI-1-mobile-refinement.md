...
- [ ] Mudar o fundo do `Header.tsx` de `bg-white/80` para `bg-[#0F0F0F]/80`.
- [ ] Alterar o texto e ícones do Header para `text-[#F8F8F8]`.
- [ ] Remover a borda inferior clara ou trocar por `border-white/10`.
- [ ] O ícone do dashboard deve usar a cor `primary` (#5DD62C) em vez de `blue-600`.

### 2. Dashboard (Responsividade e Tipografia)
- [ ] **Visão Geral (h2):** Mudar de `text-5xl` para `text-3xl` em mobile e `text-5xl` em desktop.
- [ ] **Lucro Líquido (Valor):** Mudar de `text-6xl` para `text-4xl` em mobile.
- [ ] **Textos Secundários:** Mudar classes `text-slate-500` e `text-slate-400` para cores com mais contraste onde necessário (ex: `text-slate-400` ou `text-slate-300`).
- [ ] **Paddings:** Reduzir `p-10` e `p-8` nos cards para `p-6` em dispositivos móveis (`sm:p-10 p-6`).

### 3. Global CSS
- [ ] Revisar a classe `.btn-primary`: reduzir altura (`h-16` para `h-14`) e tamanho da fonte em mobile.
- [ ] Revisar `.card-morph`: ajustar padding para ser responsivo.
